...
public class MiniHudRenderer {
    ...
    public static void render(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        if (CatalystConfig.getInstance().miniHudShowCoords) {
            ...
            guiGraphics.drawString(mc.font, coords, x, y, color, true);
            y += lineHeight;
        }
        ...
        if (CatalystConfig.getInstance().miniHudShowBiome) {
            ...
            guiGraphics.drawString(mc.font, Component.translatable("catalyst.hud.biome", biomeName).getString(), x, y, color, true);
            y += lineHeight;
        }
        ...
        if (CatalystConfig.getInstance().miniHudShowTime) {
            ...
            guiGraphics.drawString(mc.font, Component.translatable("catalyst.hud.time", String.format("%02d:%02d", hours, minutes)).getString(), x, y, color, true);
            y += lineHeight;
        }

        if (CatalystConfig.getInstance().miniHudShowDayCount) {
            long dayCount = level.getDayTime() / 24000L + 1;
            guiGraphics.drawString(mc.font, Component.translatable("catalyst.hud.day", dayCount).getString(), x, y, color, true);
            y += lineHeight;
        }
        ...
        if (CatalystConfig.getInstance().miniHudShowEntityCount) {
            ...
            guiGraphics.drawString(mc.font, Component.translatable("catalyst.hud.entities", entityCount).getString(), x, y, color, true);
        }
    }
}
